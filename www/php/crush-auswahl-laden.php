...
if (loginÜberprüfen($user_login)) {
    ...
    /////////////////// Crush Angaben vom Benutzer laden

    $love = array();

    foreach ($benutzer as $benutzername) {

        $command = "SELECT * FROM crushcount WHERE Benutzername = '$usernameToLoad'";
        $sql = sqlCommand($command);
        ...
        if ($sql->num_rows > 0) {
            while ($row = $sql->fetch_assoc()) {
                ...
                if ($benutzername != $usernameToLoad) { // Nicht sich selber laden
                    ...
                    array_push($love, $row[$benutzername]); // Einzelnen nummern laden
    ...
    for ($i = 0; $i < $count; $i++) {
        echo "☾".$benutzer[$i]."✩".$nachname[$i]."✩".$vorname[$i]."✩".$sex[$i]."✩".$love[$i];
    }
}
